    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as file:
            with mmap.mmap(file.fileno(), length=0, access=mmap.ACCESS_READ) as mm:
                return mm.find(target_string.encode('utf-8')) != -1
...
def insertar_linea_en_posicion(ruta_archivo):
    try:
        linea_deseada = "ID AÑO MES " + " ".join(map(str, range(1, 32))) + "\n"
    ...
    for col in df.columns[1:]:
        for idx, value in enumerate(df[col]):
            try:
                ...
                invalid_values.append(f"Invalid value '{value}' at row {idx}, column '{col}' in file '{filename}'")
    if invalid_values:
        logging.warning("\n".join(invalid_values))
